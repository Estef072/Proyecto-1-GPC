...
def flat(render, **kwargs):
    ...
    intensity = lbm.dot(triangleNormal, [-dirLight.x,-dirLight.y,-dirLight.z])
...
def glow(render, **kwargs):
    ...
    glowAmount = 1 - lbm.dot(triangleNormal, camForward)
...
def normalMap(render, **kwargs):
    ...
    if render.normal_map:
        ...
        texNormal = [texNormal[0] * 2 - 1,
                     texNormal[1] * 2 - 1,
                     texNormal[2] * 2 - 1]
        texNormal[:] = [x / lbm.norm(texNormal) for x in texNormal]
        ...
        texNormal = lbm.matMultVect(tangentMatrix, texNormal)
        texNormal[:] = [x / lbm.norm(texNormal) for x in texNormal]

        intensity = lbm.dot(texNormal, [-dirLight.x,-dirLight.y,-dirLight.z])
    else:
        intensity = lbm.dot(triangleNormal, [-dirLight.x,-dirLight.y,-dirLight.z])
